void moteur(int arriveeTele, int borneEN){
  
if ((arriveeTele > valeurTelBasse && arriveeTele < valeurTelHaute))
  {
   analogWrite(borneEN, 0);  
  }
else {
  //sens avant
  ...
  analogWrite(borneEN, abs(map(arriveeTele,valeurTelHaute,2000,0,255)));  
    }
  }
   //sens arriere
  ...
  analogWrite(borneEN, abs(map(arriveeTele,valeurTelBasse,1000,0,255)));  
